...
function [MTS, MTSp, nMTS, nMTSp] = calcMtess_c(CX, range, nDft, pccFunc, ccLags, pccLags, CXNames, cachePath)
    ...
    [sz1, sz2] = size(CX{1});
    procf = [cachePath '/mtess-' CXNames{1} '-' num2str(sz1) 'x' num2str(sz2) 'd' num2str(nDft) 'c' num2str(ccLags) palgo num2str(pccLags) '-proc.mat'];
    ...
    for i=1:cLen
        ...
        parfor j=i+1:cLen
            ...
            [sz1, sz2] = size(CX{j});
            cachef = [cachePath '/mtess-' CXNames{j} '-' num2str(sz1) 'x' num2str(sz2) 'd' num2str(nDft) 'c' num2str(ccLags) palgo num2str(pccLags) '.mat'];
            fj = load(cachef);
