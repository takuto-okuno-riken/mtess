...
function Y = surrogateLazyLearning_(X, exSignal, nodeControl, exControl, LL, kn, dist, surrNum)
    ...
    if ~isempty(LL.Y)
        ...
        %    parfor n=1:nodeNum
                ...
                Xti = single(nan(allInLen,length(idxn{n})));
                xts = 1;
                ...
                for i=1:cxNum
                    ...
                    Xt(xts:xts+sl-1,:) = Z(1:sl,n);
                    Xti(xts:xts+sl-1,:) = Yj(:,idxn{n});
                    xts = xts + sl;
                end
                Z = [];  % clear memory
                Yj = []; % clear memory
                ...
                % predict and get errors
                A = predictLazyLearning(Xti.', Xti.', Xt.', kn);
                Err(n,:) = Xt.' - A;
            end
        end
    end
...
%                parfor i=1:nodeNum
            ...
            %    parfor n=1:nodeNum
                    ...
                    % yield next time step
                    A(n) = predictLazyLearning(S2, Xti.', Xt.', kn) + noise2(n);
                end
            end
            S(:,t) = A;
        end
