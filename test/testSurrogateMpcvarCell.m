...
function testSurrogateMpcvarCell
    % load signals
    ...
    for lags=1:5
        net = initMpcvarNetworkWithCell(CX, [], [], [], lags, 0.99, 0.01);
        Y1 = surrogateMpcvar(X, [], [], [], net, 'gaussian', 10);
        Y2 = surrogateMpcvar(X, [], [], [], net, 'residuals', 10);
        net = initMvarNetworkWithCell(CX, [], [], [], lags, 0.01);
        Y3 = surrogateMVAR(X, [], [], [], net, 'gaussian', 10);
        Y4 = surrogateMFT(X, 10);

        figure; plotMultiSignals(cat(3,X,Y1(:,:,3),Y1(:,:,2)));
        figure; plotMultiSignals(cat(3,X,Y2(:,:,3),Y1(:,:,2)));
        figure; plotMultiSignals(cat(3,X,Y3(:,:,3),Y1(:,:,2)));
        figure; plotMultiSignals(cat(3,X,Y4(:,:,3),Y1(:,:,2)));
        ...
        S(:,:,2) = Y1(:,:,3);
        checkSimilarity(S, 1, 100, 'surrogate Mpcvar-gaussian');
        S(:,:,2) = Y2(:,:,3);
        checkSimilarity(S, 1, 100, 'surrogate Mpcvar-residuals');
        S(:,:,2) = Y3(:,:,3);
